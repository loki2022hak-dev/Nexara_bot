...
async def ai_analyze(query, raw_data):
    ...
    prompt = f"Ти — OSINT система NEXARA. Об'єкт: {query}. Дані: {str(raw_data)}. Склади ПОВНЕ ДОСЬЄ: ПІБ, авто, сім'я, робота, реєстри, борги. Українською мовою."
    async with aiohttp.ClientSession() as s:
...
async def start(m: Message):
    await m.answer("🛡️ <b>NEXARA CORE SYSTEM ONLINE</b>", reply_markup=nexara_menu())
...
async def pdf_gen(m: Message):
    s = get_stats(m.from_user.id)
    if not s: return await m.answer("Немає даних.")
    path = DOSSIER_DIR / f"nexara_{m.from_user.id}.pdf"
    c = canvas.Canvas(str(path), pagesize=A4)
    c.drawString(50, 800, f"NEXARA REPORT: {s[3]}")
    y = 780
    for l in json.loads(s[4]):
        c.drawString(50, y, str(l)); y -= 15
...
async def engine(m: Message):
    if m.text in ["🔍 Новий пошук", "📂 Мої результати", "💎 VIP / Тарифи", "🆘 Підтримка"]:
        return await m.answer("Введіть ПІБ, Нікнейм або IP:")

    if any(x.lower() in m.text.lower() for x in FORBIDDEN_DATA) and m.from_user.id != ADMIN_ID:
        return await m.answer("❌ Захищено.")

    st = await m.answer("📡 <b>NEXARA: Глибокий пошук...</b>")
    query = m.text.strip()
    
    if re.match(r"^\d{1,3}(\.\d{1,3}){3}$", query):
        res = await run_shodan(query)
    else:
        sh, mg = await asyncio.gather(run_sherlock(query), run_maigret(query))
        res = list(set(sh + mg))

    save_res(m.from_user.id, query, res)
    report = await ai_analyze(query, res)
    
    await st.delete()
    await m.answer(f"📊 <b>РЕЗУЛЬТАТ: {query}</b>\n\n{report}", reply_markup=nexara_menu())
